...
class Route
{
    ...
    public function dispatch(Request $request, $withRoute = null)
    {
        ...
        //  这里实际是调用了think\route\dispatch\Controller与抽象类think\route\Dispatch的init方法
        //  其中在Dispatch的doRouteAfter方法中对中间件类型route做了引入
        $dispatch->init($this->app);
        //  如果在上一步的init调用中引入了route类型的中间件,则进行相关中间件调用.
        //  最终执行Dispatch的run方法, 并调用子类(Controller)的exec方法(返回具体的响应体Response).
